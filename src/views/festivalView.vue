<template>
<main class="py-36">
    <p class=" mt-8 mb-4 w-full text-[33px] bg-[#e6f2dd] font-bold text-center text-black">festival</p>

     <H2 class="font-manuale text-center text-xs p-2 text-2xl leading-8">DÉCOUVREZ L’ALKEBULAN FESTIVAL !</H2>

     <div class="ml-5 gap-5 flex w-[27%]">

       <img src="/image/6.png" alt="1">
       <img src="/image/7.png" alt="2">
       <img src="/image/8.png" alt="1">

</div>
 <H4 class="p-8 leading-[3rem]  text-center text-2xl  font-codystar">4 JOURS INCROYABLES</H4>

  <div class="relative w-max mx-auto">
      <svg class="absolute top-0 left-0 w-full h-full"
  width="170"
  height="48"
  viewBox="0 0 170 48"
  fill="none"
  xmlns="http://www.w3.org/2000/svg"
  preserveAspectRatio="none"
>
  <path d="M16.8229 0H170L158.596 48H0L16.8229 0Z" fill="#FFF8F8" fill-opacity="0.8" class="text-2xl font-extrabold ">
  </path>
  
</svg>
<p class="relative p-3 w-max text-black font-extrabold" >DU 4 AU 7 AOÛT 2022</p>
    </div>

     <H4 class="p-8 leading-[3rem]  text-center text-2xl  font-codystar">L’AFROBEAT VISITE LA CAPITALE</H4>
<div class="text-center">
      <div class="bg-purple-800 ml-9 inline-block p-2 rounded-2xl m-auto mt-3 mb-4">
        <a href="/"  class="block font-manuale text-xs ">JE VEUX MA PLACE</a>
      </div>
      </div>

<div class="text-center">
      <h3 class="font-bold ">Qu’est ce que le festival alkebulan?</h3>
      </div>
      <p class="ml-4 mr-4 mt-4 font-manuale text-[14px]">C’est un Festival de musique à caractère afrobeat comprenant des stars internationales tel que Beyonce, mais ce festival à une caractéristique particulière: il recevra uniquement des artistes africain produisant de la musique comportant de l'afrobeats. Aya Nakamura, Rema, Wizkid et bien d’autres encore pourront exposer leur talents à travers ce festival. </p>


<div class="flex flex-col items-center">
<img class="mt-4" src="/image/rond1.png" alt="">

<div class="flex gap-9 mt-7 " >
<img class="" src="/image/rond2.png" alt="">
<img class="" src="/image/rond3.png" alt="">

</div>
</div>

</main>
</template>